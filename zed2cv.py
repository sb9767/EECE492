...
def main() :
    ...
    init.depth_maximum_distance = 1000
    ...
    runtime.confidence_threshold = 45
    ...
    # Connection to CV ======= sby ======= #

    # width = image_ocv.get(cv2.CAP_PROP_FRAME_WIDTH)
    # height = image_ocv.get(cv2.CAP_PROP_FRAME_HEIGHT)

    # fourcc = cv2.VideoWriter_fourcc(*'DIVX')
    # out = cv2.VideoWriter('output.avi', fourcc, 30.0, (int(image_size.width), int(image_size.height)))
    ...
    while key != 113 :
        ...
        if err == sl.ERROR_CODE.SUCCESS :
            ...
            blur = cv2.GaussianBlur(depth_image_ocv, (blurValue, blurValue), 0)
            ret, thresh = cv2.threshold(blur, threshold, 255, cv2.THRESH_TOZERO)
            ...
            # record = True
            # video = cv2.VideoWriter("D:/" + str(now) + ".avi", fourcc, 20.0, (depth_image_ocv.shape[1], depth_image_ocv.shape[0]))
            # # out.write(thresh)
            # if record == True:
            #     print("녹화 중..")
            #     video.write(depth_image_ocv)
